...
${CYAN}명령어:${NC}
    ...
    status          현재 상태 확인
    logs [서비스]   로그 보기 (vllm / reranker / 미지정=전체)
    restart [서비스] 재시작 (vllm / reranker / 미지정=전체)

${CYAN}예시:${NC}
    $0                  # 전체 종료
    $0 llm              # LLM만 종료 (reranker 유지)
    $0 reranker         # Reranker만 종료
    $0 status           # 상태 확인
    $0 logs vllm        # LLM 로그
    $0 logs reranker    # Reranker 로그
    $0 restart reranker # Reranker만 재시작
...
status() {
    ...
    # ── LLM 상태 ──
    echo -e "  ${CYAN}[LLM]${NC}"
    local llm_running=$(docker compose ps -q vllm 2>/dev/null)
...
" 2>/dev/null || true
    ...
    local rr_running=$(docker compose ps -q reranker 2>/dev/null)
...
main() {
    ...
    case "$cmd" in
        ...
        restart)
            ...
            ;;
        llm)
            log_info "LLM 서비스 종료 (reranker는 유지)..."
